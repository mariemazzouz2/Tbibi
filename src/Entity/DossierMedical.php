...
use App\Entity\Utilisateur;  // Ensure Utilisateur is also imported
...
private ?string $unite = null;
     ...
     */
    #[ORM\OneToMany(targetEntity: Prediction::class, mappedBy: 'dossier')]
    ...
    public function removePrediction(Prediction $prediction): static
    {
        if ($this->predictions->removeElement($prediction)) {
            // set the owning side to null (unless already changed)
            if ($prediction->getDossier() === $this) {
                $prediction->setDossier(null);
            }
        }
